...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    */
    ...
    std::vector<JSAMPLE> temp;
    JSAMPARRAY t;

    temp.resize(row_stride);
    ...
    while (cinfo.next_scanline < cinfo.image_height) {
        ...
        for (int i = 0, j = 0; i < image.GetWidth(); i++) {
            temp[j] = static_cast<JSAMPLE>(image.GetPixel(i, cinfo.next_scanline).r);
            temp[j + 1] = static_cast<JSAMPLE>(image.GetPixel(i, cinfo.next_scanline).g);
            temp[j + 2] = static_cast<JSAMPLE>(image.GetPixel(i, cinfo.next_scanline).b);
            j += 3;
        }
